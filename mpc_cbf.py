...
class MPCCBF:
    def __init__(self, robot, robot_spec):
        ...
        if self.robot_spec['model'] == 'Unicycle2D':
            ...
            self.Q = np.diag([50, 50, 30, 30, 0.01])  # State cost matrix
            self.R = np.array([0.5, 0.5])  # Input cost matrix
    ...
    def create_model(self):
        ...
        # Parameters
        _goal = model.set_variable(var_type='_tvp', var_name='goal', shape=(self.n_states, 1))
    ...
    def set_tvp(self, mpc):
        ...
        def tvp_fun(t_now):
            ...
            if self.obs is None:
                ...
                tvp_template['_tvp', :, 'obs'] = np.concatenate([self.goal+1000, [0]])
            else:
    ...
    def set_cbf_constraint(self, mpc):
        ...
        cbf_constraint = self.compute_cbf_constraint(_x, _u, _obs) # here, use symbolic variable
    ...
    def compute_cbf_constraint(self, _x, _u, _obs):
        ...
        else:  # DoubleIntegrator2D
            # Implement the appropriate CBF for DoubleIntegrator2D if needed
            cbf_constraint = 0
    ...
    def solve_control_problem(self, robot_state, control_ref, nearest_obs):
        ...
        if nearest_obs is not None:
            cbf_constraint = self.compute_cbf_constraint(x_next, u, nearest_obs) # here use actual value, not symbolic
