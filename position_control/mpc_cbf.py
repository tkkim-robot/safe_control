...
class MPCCBF:
    def __init__(self, robot, robot_spec, show_mpc_traj=False, num_obs=5):
        ...
        if self.robot_spec['model'] == 'SingleIntegrator2D':
            ...
            self.R = np.array([0.5, 5000.0])  # Input cost matrix
        ...
        if self.robot_spec['model'] == 'SingleIntegrator2D':
            ...
            self.cbf_param['alpha1'] = 0.05 # 0.35 for high parameters
    ...
    def solve_control_problem(self, robot_state, control_ref, nearest_obs):
        ...
        # if 'alpha' in self.cbf_param:
        #     print(f"[Solve start] alpha = {self.cbf_param['alpha']}")
        # else:
        #     print(f"[Solve start] alpha1 = {self.cbf_param['alpha1']}, alpha2 = {self.cbf_param['alpha2']}")
